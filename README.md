...
void setup(){
  ...
  background(255);
  stroke(0,0,255);
  strokeWeight(16);//선의 크기
}
float f;
void draw(){
  if(mousePressed)
  line(pmouseX,pmouseY,mouseX,mouseY);//선의 좌표를 클릭마우스로 지정함
}
```
...
void draw(){
  ...
  if(i>height) i=0;
  i=i+dir*sp;
  if (keyPressed) sp = key-'0';//숫자키에 따른 속도 조절
}
```
...
void setup() {
  ...
  fill(245,245,220);//배 중심
  ellipse(500,470,100,100);

  fill(248,155,0);//왼쪽 다리
  ellipse(435,600,50,180);
 

  fill(248,155,0);//오른쪽 다리
  ellipse(565,600,50,180);

 
  stroke(248,155,0);//오른쪽팔
  strokeWeight(40);
  line(588,400,641,550);
 
  stroke(248,155,0);//왼쪽 팔
  strokeWeight(40);
...
void draw(){
 ...
 if(Layer ==1){
    drawLayer(pg1,color(200));//Layer의 값이 1일때 pg1,색은(200)
} else if (Layer ==2){
   drawLayer(pg2, color (100,200));//Layer의 값이 2일때 pg1,색은(100,200)
}
  image(pg1,0,0);
  image(pg2,0,0);
}
void keyPressed(){
  if(key == '1'){
    Layer = 1;//1을 누르면 Layer의 값이 1
} else if (key == '2'){
  Layer = 2;//2을 누르면 Layer의 값이 2
}
}
